...
resource "azurerm_container_registry" "globalacrpp1" {
...
output "acr_login_server" {
  ...
  value       = azurerm_container_registry.globalacrpp1.login_server
...
resource "azurerm_private_endpoint" "acr_private_endpoint" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_container_registry.globalacrpp1.id
...
data "azurerm_subnet" "acr-subnet" {
  ...
  resource_group_name  = azurerm_container_registry.globalacrpp1.resource_group_name
}
...
/*data "azuread_service_principal" "GlobalAzure" {
  display_name = "GlobalAzure"
}

resource "azurerm_role_assignment" "Acrpush_role" {
  scope                = azurerm_container_registry.globalacrpp1.id
  role_definition_name = "AcrPush"
  principal_id         = data.azuread_service_principal.GlobalAzure.object_id ##Object ID
}*/